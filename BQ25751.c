...
{
    // Length of the register can be either 1 or 2
    ...
    // Initializing I2C handle (gI2cHandle is an array auto generated by SysConfig)
    ...
    // Performing the transfer and putting remembering the status of it
    int32_t status = I2C_transfer(i2cHandle, &i2cTransaction);

    // Handling the faulty transaction
    ...
    if (status == I2C_STS_SUCCESS)
    {
        switch (length) // register length, could be 1 or 2 bytes
        {
            ...
            case 2:
                // If length is 2 bytes, parsing as little endian
...
int32_t BQ25751_write_reg(uint8_t register_addr, uint8_t length, uint16_t data)
{
    ...
    switch (length) // depending on the length of the register (1 or 2 bytes)
    {
        ...
        case 2:
            // If 2 bytes, correctly filling the buffer with data as little endian
            tx_buffer[1] = (uint8_t)(data & 0xFF);
...
{
    // Reading 2 fault registers and priting if we see any faults
...
{
    // Read every register and print its value
...
void PPMC_run_test_mode(void)
{
    ...
    while (true)
    {
        ...
        if ((ClockP_getTimeUsec() - BQ25751_timer) > BQ25751_TEST_TIMEOUT && BQ25751_timer_en )
        {
            ...
            // Will store the power good register value here
