...
void PDM_testing(void *args)
{
    ...
    // Always call these functions, they call the intialization stuff generated by sysConfig
    Drivers_open();
    Board_driversOpen();

    DebugP_uartSetDrvIndex(CONFIG_UART0); // Setting default uart driver, so that we can use UART serial port for debugging
    init_gpio();
    BQ25751_init();
    BQ25856_init();
    pmic_init();
    ...
    ClockP_sleep(1); // Needed for ADC to get enabled correctly

    // Initializing SPI (needed for external ADC)
    ...
    while(true)
    {
        ...
        DebugP_scanf("%d", &user_input_test_number); // Getting user input
    ...
    // Always call these functions in the end, they deinitialize stuff generated by sysConfig
    Board_driversClose();
    Drivers_close();
}
